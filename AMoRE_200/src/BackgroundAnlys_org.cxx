	...
	//for( int i = 0; i < 10000; i++ ) {
		...
		for( int ist = 0; ist < TotStepN; ist++ ){
			...
			if( strstr(vname, "CrystalCell") ) {
				const char *towerNumberStr = strtok(buffer,"_");
				...
				if(nowEdep <= 0.) continue;

				stepTotEdep += nowEdep;
				if( strstr(pname,"alpha") || pdgcode>20040 ){
					if( readSingleStep->GetGlobalTime() <= 5000000. ){	edepAlpha[crystalNum] += nowEdep;	} // 5ms cut
				}
				else {
					if( readSingleStep->GetGlobalTime() <= 5000000. ){	edepBeta[crystalNum] += nowEdep; }
				}
			}
		}
