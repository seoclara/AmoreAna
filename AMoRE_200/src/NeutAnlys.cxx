...
void NeutAnlys(const int runno, const char *path, const char *fileformat, const char *outpath, const int numpertask) 
{
#ifndef __CLING__
	...
	Double_t edepWT;
	Double_t edepPST[number_of_scintillators];
	Double_t edepPSO[number_of_scintillators];
	Double_t edepPSI[number_of_scintillators];
	...
	TTree *fOTree  = new TTree("NeutAnlys", "Neut analysis production", 5);
	//// file information
	...
	fOTree->Branch(Form("EdepPS_total[%d]",number_of_scintillators), &edepPST,Form("EdepPS_total[%d]/D",number_of_scintillators));
	fOTree->Branch(Form("EdepPS_outer[%d]",number_of_scintillators), &edepPSO,Form("EdepPS_outer[%d]/D",number_of_scintillators));
	fOTree->Branch(Form("EdepPS_inner[%d]",number_of_scintillators), &edepPSI,Form("EdepPS_inner[%d]/D",number_of_scintillators));
	fOTree->Branch("WTTouched", &WTTouched, "WTTouched/O");       // Direct hit on WT
	fOTree->Branch("VetoTouched", &VetoTouched, "VetoTouched/O"); // Direct hit on PS
	fOTree->Branch("CrystalHitted", &CMOHitted, "CrystalHitted/O");
	fOTree->Branch("SingleHit", &SingleCMOHit, "SingleHit/O");
	fOTree->Branch("ParticleColl", &AssociatedParticleColl, 1000000); // Collection of particle coll
	fOTree->Branch("CrystalStepColl", &CMOStepColl, 1000000);             // Collection of steps
	fOTree->Branch("CrystalHitColl", &CMOHitColl, 1000000);               // Collection of hits
	...
	for (i = 0; i < entries; i++) { //// start event loop
		...
		if (TotStepN > 0) {
			...
			for (j = 0; j < TotStepN; j++) {
				...
				if(readSingleStep->GetTrackID() == 1 && strstr(readSingleStep->GetVolumeName(), "CrystalCell")) CMOHitted = true;
				...
				if(strstr(readSingleStep->GetVolumeName(), "HatWaterTank") && readSingleStep->GetEnergyDeposit()>0){
					...
					TString ps_layer( volumename(12,1) );
					int id = found.Atoi();
					if( strstr(ps_layer,"O") ) edepPSO[id] += readSingleStep->GetEnergyDeposit();
					else if( strstr(ps_layer,"I") ) edepPSI[id] += readSingleStep->GetEnergyDeposit();
					edepPST[id] = edepPSO[id]+edepPSI[id];
				}
				
				//// save the associated particle which make crystal hit
				if (strstr(readSingleStep->GetVolumeName(), "physCrystalCell") != NULL) {
					Int_t towernum;
					Int_t floornum;
					sscanf(readSingleStep->GetVolumeName(), "physCrystalCell%d_%d", &towernum, &floornum);
					Int_t cmonum = towernum*9+floornum;
...
int main(int, char *argv[]) {
	NeutAnlys(std::stoi(argv[1]), argv[2], argv[3], argv[4], stoi(argv[5]));
	return 0;
}
